...
import { getMealsByCategory } from "@/lib/mealdb/client";
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const meals = await getMealsByCategory(categoryName).catch(() => null);
  ...
  return (
    <div className="flex flex-1 justify-center bg-surface-warm px-4 py-12">
      <div className="flex w-full max-w-5xl flex-col gap-6">
        ...
        {meals.length === 0 ? (
          <p className="text-sm text-surface-text-muted">Bu kategoride tarif bulunamadı.</p>
        ) : (
